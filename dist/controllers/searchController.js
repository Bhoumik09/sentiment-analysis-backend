...
const search = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (industry) {
        conditions.push(client_1.Prisma.sql `s.sector ILIKE ${`%${industry}%`}`);
    }
    if (sentimentScoreLimit !== undefined) {
        conditions.push(client_1.Prisma.sql `stats.avg_sentiment_score >= ${Number(sentimentScoreLimit)}`);
    }
    if (sentiment) {
        if (sentiment === "positive")
            conditions.push(client_1.Prisma.sql `stats.avg_sentiment_score > .01`);
        if (sentiment === "negative")
            conditions.push(client_1.Prisma.sql `stats.avg_sentiment_score < -0.01`);
        // A slightly cleaner way to write the neutral condition
        if (sentiment === "neutral")
            conditions.push(client_1.Prisma.sql `stats.avg_sentiment_score BETWEEN -0.01 AND 0.01`);
    }
